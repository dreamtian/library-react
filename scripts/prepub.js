...
if (fs.existsSync(path.join(__dirname, '../dist'))) {
  // Build a entry less file to dist/${packageInfo.name}.less
  ...
  fs.readdir(componentsPath, function (err, files) {
    files.forEach(function (file) {
      if (fs.existsSync(path.join(componentsPath, file, 'style', 'index.less'))) {
        componentsLessContent += `@import "../${file}/style/index.less";\n`
      }
    });
    fs.writeFileSync(path.join(process.cwd(), 'lib', 'style', 'components.less'), componentsLessContent);

    // Build less entry file: dist/${packageInfo.name}.less
    fs.writeFileSync(
